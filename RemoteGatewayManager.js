...
class RemoteGatewayManager {
    ...
    registerWebSocket(gatewayId, ws, ipAddress) {
        ...
        this.plugin.debug(`RemoteGateway: WebSocket registered for ${gatewayId}${ipAddress ? ` (${ipAddress})` : ''}`)
        return gattManager
    }
    ...
    async _handleOneAdvertisement(adv, gatewayId) {
        ...
        if (this.emitBLEAdvertisement) {
            ...
            const name = (sensor && typeof sensor.getName === 'function') ? sensor.getName() : adv.name
            const mfrHex = {}
            for (const [id, buf] of Object.entries(mfrData)) {
                mfrHex[parseInt(id)] = buf.toString('hex')
            }
            this.emitBLEAdvertisement(mac, name, adv.rssi, mfrHex, `remote:${gatewayId || 'unknown'}`)
        }
    }
}
